...
The site is four static files. There is no server code, no database server and nothing to install at
...
`vendor/` must stay a folder beside `index.html`, and `list.sqlite` must sit next to `index.html`:
...
which appears at the foot of the page with the build date. **`index.html` is generated — never edit
it by hand**; edit the parts in `site/` and build again.
...
Copy the four files, keeping the folder shape:
...
plain Apache or nginx directory. For a leaner site than the repository root, publish only those four
