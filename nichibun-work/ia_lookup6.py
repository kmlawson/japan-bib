#!/usr/bin/env python3
"""archive.org lookups for the Nichibunken catalogue books that are not already in the database.

Same search and cache format as next-bib-work/ia_lookup2.py; the cache is ia_cache5.jsonl and the run
is resumable, so it can be repeated as more pages are transcribed.

    ia_lookup6.py [--workers 3]
"""
...
import nichi_merge as D  # noqa: E402
...
if __name__ == "__main__":
    ...
    def work(kr):
        ...
        with lock:
            with open(D.CACHE6, "a", encoding="utf-8") as f:
