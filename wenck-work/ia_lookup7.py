#!/usr/bin/env python3
"""archive.org lookups for the Wenckstern (1907) books that are not already in the database.

Same search and cache format as next-bib-work/ia_lookup2.py; the cache is ia_cache7.jsonl and the run
is resumable, so it can be repeated as more pages are transcribed.

    ia_lookup7.py [--workers 3]
"""
...
import wenck_merge as W  # noqa: E402

lock = threading.Lock()


def rows_from_db():
    """The database without the rows Wenckstern himself contributed - otherwise, once it has been
    rebuilt, every Wenckstern entry would look like a duplicate of itself and nothing would be looked up."""
    ...
    rows = [list(x) for x in con.execute(
        ...
        "WHERE source <> ? ORDER BY id", (W.SRC,))]
    con.close()
    return rows
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    _, new = W.split(rows_from_db())
    cache = M.load_cache2(W.CACHE7)
    todo = {}
    for r in new:
    ...
    def work(kr):
        ...
        with lock:
            with open(W.CACHE7, "a", encoding="utf-8") as f:
